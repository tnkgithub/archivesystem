...
som = pd.read_csv(BASE_DIR / 'system/csvs/image_som_result20230110_073816.csv', index_col=0)
list_som = []
for i in som.index:
    list_som.append(i)

# 代表資料(扱いやすくするためリストに変換)
represent_image = pd.read_csv(BASE_DIR / 'system/csvs/rep_image_som_result_3-3.csv', index_col=0)
rep_image = []
for i in represent_image.index:
    rep_image.append(i)

# 画像ファイル名→タイトル変換用辞書
image_title = pd.read_csv(BASE_DIR / 'system/csvs/imageName_title_rename_dict.csv', index_col=0).to_dict()

# 画像ファイル名→タイトル(管理番号あり)変換用辞書
image_title_num = pd.read_csv(BASE_DIR / 'system/csvs/imageName_title_dict.csv', index_col=0).to_dict()

# 画像ファイル名→デジタル資料館の説明画面リンク変換用辞書
links = pd.read_csv(BASE_DIR / 'system/csvs/imageName_link_rename_dict.csv', index_col=0).to_dict()

# タイトル(管理番号あり)→画像ファイル名変換用辞書
title_image = pd.read_csv(BASE_DIR / 'system/csvs/title_imageName_rename_dict.csv', index_col=0).to_dict()

# 画像のベクトル表現
image_features = pd.read_csv(BASE_DIR / 'system/csvs/rename_featuresTitle_pca_350_MinMax.csv', index_col=0)

# タイトルのベクトル表現
title_features = pd.read_csv(BASE_DIR / 'system/csvs/titleVectorsNoANDNumber.csv', index_col=0)
...
def SOM(request):
    ...
    if "imageID" in request.GET:
        ...
        ctx["spiral_dict"] = to_spiral(id, sorted_dict)
        ctx["sorted_dict"] = to_part(id, sorted_dict)
...
def sort_image_Similarity(id):
    # ソート後の辞書
    sorted_dict = {}
    sorted_dict[id] = 1.0 # 自分自身の類似度は1.0
    # 類似度を計算し、辞書を作成
